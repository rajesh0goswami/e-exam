...
String sql ="select * from sub2qs where qsno="+'\''+no+'\'';
...
%>
<form action = "./Evaluate2?no=<%=resultSet.getInt("qsno") %>&user=<%=user %>" method = "post">
...
<form action="./Clear2?no=<%=resultSet.getInt("qsno") %>&user=<%=request.getParameter("user") %>" method="post">
