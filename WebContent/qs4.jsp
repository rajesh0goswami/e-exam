...
String sql ="select * from sub4qs where qsno="+'\''+no+'\'';
...
%>
<form action = "./Evaluate4?no=<%=resultSet.getInt("qsno") %>&user=<%=user %>" method = "post">
...
<form action="./Clear4?no=<%=resultSet.getInt("qsno") %>&user=<%=request.getParameter("user") %>" method="post">
<input type= "submit" value = "Clear Response" id="clr" name="clr">
</form>
<form action="./Marks4?user=<%=user %>" method="post">
