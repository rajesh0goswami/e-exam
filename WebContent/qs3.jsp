...
String sql ="select * from sub3qs where qsno="+'\''+no+'\'';
...
%>
<form action = "./Evaluate3?no=<%=resultSet.getInt("qsno") %>&user=<%=user %>" method = "post">
...
<form action="./Clear3?no=<%=resultSet.getInt("qsno") %>&user=<%=request.getParameter("user") %>" method="post">
<input type= "submit" value = "Clear Response" id="clr" name="clr">
</form>
<form action="./Marks3?user=<%=user %>" method="post">
